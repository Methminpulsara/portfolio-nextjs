...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl
        .to(items[0], { duration: 1, scale: 1.02, ease: "sine.inOut" })
        .to(items[0], { duration: 1, opacity: 0, y: -30, scale: 0.95, ease: "power2.in" }, "+=0.8")
        .to(items[1], { duration: 1, opacity: 1, y: 0, scale: 1, ease: "power2.out" }, "-=0.5")
        .to(items[1], { duration: 1, scale: 1.02, ease: "sine.inOut" })
        .to(items[1], { duration: 1, opacity: 0, y: -30, scale: 0.95, ease: "power2.in" }, "+=0.8")
        .to(items[2], { duration: 1, opacity: 1, y: 0, scale: 1, ease: "power2.out" }, "-=0.5")
        .to(items[2], { duration: 2, scale: 1.02, ease: "sine.inOut", repeat: 1, yoyo: true });
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        <div className="wave-line" style={{ top: '40%', animationDelay: '1s' }} />
        <div className="wave-line" style={{ top: '60%', animationDelay: '2s' }} />
        <div className="wave-line" style={{ top: '80%', animationDelay: '3s' }} />
      </div>

      <div className="relative z-50 w-full px-6 pt-8 flex justify-end">
        <div 
          onClick={copyEmail}
          className="group relative flex items-center gap-3 px-6 py-3 bg-black/50 backdrop-blur-2xl border border-white/20 rounded-full cursor-pointer overflow-hidden transition-all duration-700 hover:scale-110 hover:border-emerald-400/60"
        ...
        >
          ...
          <div className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse absolute" />
      ...
      <div ref={textContainerRef} className="relative z-20 flex-1 flex items-center justify-center px-6 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-8">
            ...
            <div className="flex flex-wrap gap-4 text-sm font-mono order-1 md:order-2">
              <div className="px-6 py-3 bg-neutral-900/50 backdrop-blur-xl rounded-2xl border border-neutral-800 transition-all duration-500 hover:border-emerald-500/50 cursor-default">
                <span className="text-neutral-400 hover:text-white transition-colors tracking-wider uppercase">Sri Lanka</span>
              </div>
              
              <div className="px-6 py-3 bg-neutral-900/50 backdrop-blur-xl rounded-2xl border border-neutral-800 transition-all duration-500 hover:border-emerald-500/50 cursor-default">
                <div className="flex items-center gap-3">
                  <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse-slow" />
                  <span className="text-neutral-400 hover:text-white transition-colors tracking-wider uppercase">{currentTime}</span>
                </div>
              </div>
              
              <div className="px-6 py-3 bg-neutral-900/50 backdrop-blur-xl rounded-2xl border border-neutral-800 transition-all duration-500 hover:border-emerald-500/50 cursor-default">
                <span className="text-neutral-400 hover:text-white transition-colors tracking-wider uppercase">Available</span>
              </div>
            </div>
          </div>
        </div>
